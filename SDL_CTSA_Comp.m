...
if strfind(SDL.out(end-15:end),'Increase')
    [~,kk] = sort(d,'descend'); % kk is the list of rank according to d-value (Cohen's d),from increased CT/SA to decreased CT/SA
elseif strfind(SDL.out(end-15:end),'Reduction')
    [~,kk] = sort(d,'ascend'); % kk is the list of rank according to d-value (Cohen's d),from decreased CT/SA to increased CT/SA
elseif strfind(SDL.out(end-15:end),'NoDiff')
