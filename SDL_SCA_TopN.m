...
Nr = SDL.Nr; % number of searching matched solutions, 300 for publicaiton purpose
for i = 2:length(kk) % per top-N
    ...
    k = 0; 
    tic; % beginning of the stop watch (unit: sec)
    while (k < Nr) && (signrank(dlist-md) < 0.05) && (toc < 120) % loop<300 times & within 2 min (i.e. 120 sec) if the mean distance of random nodes is different from original nodes distance
        k = k + 1;
...
for i = 2:size(SC0,2) % per top-N
    ...
    SC0(i).mean1raw = mean(SC0(i).Z1c); % mean of all connections, 1X1, original data, group 1
    SC0(i).mean2raw = mean(SC0(i).Z2c); % mean of all connections, 1X1, original data, group 2
    SC0(i).mean1pos = mean(SC0(i).Z1c(SC0(i).Z1c>0)); % mean of all positive connections, 1X1, original data, group 1
    SC0(i).mean2pos = mean(SC0(i).Z2c(SC0(i).Z2c>0)); % mean of all positive connections, 1X1, original data, group 2
    SC0(i).mean1neg = mean(SC0(i).Z1c(SC0(i).Z1c<0)); % mean of all negative connections, 1X1, original data, group 1
    SC0(i).mean2neg = mean(SC0(i).Z2c(SC0(i).Z2c<0)); % mean of all negative connections, 1X1, original data, group 2
    SC0(i).mean1abs = mean(abs(SC0(i).Z1c)); % mean of all absolute values of connections, 1X1, original data, group 1
    SC0(i).mean2abs = mean(abs(SC0(i).Z2c)); % mean of all absolute values of connections, 1X1, original data, group 2
...
fn = fullfile(fdir,['Results_TopN_SC_CI_p_PTSD_vs_CONT_',SDL.data_type{1},'_',SDL.ana_type{1},'.mat']);
load(fn);
data = zeros(5001,147,8); % 5001(1 actual + 5000 random) x 147(network size: 2~148) x 8(PTSD/non-PTSD groups x raw/pos/neg/abs)
for j = 1:147 % per network size (2~148)
    ...
    data(1,j,3) = SC0(j+1).mean1pos; data(2:5001,j,3) = SC1(j+1).mean1pos; % mean1pos
    data(1,j,4) = SC0(j+1).mean2pos; data(2:5001,j,4) = SC1(j+1).mean2pos; % mean2pos
    data(1,j,5) = SC0(j+1).mean1neg; data(2:5001,j,5) = SC1(j+1).mean1neg; % mean1neg
    data(1,j,6) = SC0(j+1).mean2neg; data(2:5001,j,6) = SC1(j+1).mean2neg; % mean2neg
    data(1,j,7) = SC0(j+1).mean1abs; data(2:5001,j,7) = SC1(j+1).mean1abs; % mean1abs
    data(1,j,8) = SC0(j+1).mean2abs; data(2:5001,j,8) = SC1(j+1).mean2abs; % mean2abs
end
...
% fprintf('\n\nFDR correction: PTSD vs random\n')
% [~, ~, ~, adj_p1] = fdr_bh(p1,0.05,'pdep','yes'); 
% find(adj_p1<=0.05)
% fprintf('\n\nFDR correction: CONT vs random\n')
% [~, ~, ~, adj_p2] = fdr_bh(p2,0.05,'pdep','yes');
% find(adj_p2<=0.05)
% fprintf('\n\nFDR correction: PTSD-CONT vs random\n')
...
% savefig(fullfile(SDL.out,SDL.data_type{1},['Results_2Dmap_',SDL.data_type{1},'_',SDL.ana_type{1},'.fig']));
% 
% 
% %% Plot correlation
...
% text(0,0.015,txt,'FontSize',18);
% 
% savefig(fullfile(SDL.out,SDL.data_type{1},['Results_Corr_',SDL.data_type{1},'_',SDL.ana_type{1},'.fig']));
% 
% % Only neg efect size
% get(0,'Factory'); set(0,'defaultfigurecolor',[1 1 1]); % change background and marginal areas color into white
% figure('DefaultAxesFontSize',18)
...
% text(-0.1,0.022,txt,'FontSize',18);
% 
% savefig(fullfile(SDL.out,SDL.data_type{1},['Results_Corr_NegEffectSize_',SDL.data_type{1},'_',SDL.ana_type{1},'.fig']));
